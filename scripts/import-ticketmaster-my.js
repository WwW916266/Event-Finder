...
ticketmasterUrl.searchParams.set("countryCode", "MY");
...
async function main() {
  ...
  if (!ticketmasterEvents.length) {
    console.log("No Ticketmaster Malaysia events found.");
    return;
  }
...
async function upsertSource() {
  const rows = await supabaseFetch("/rest/v1/event_sources?on_conflict=name", {
    ...
    body: JSON.stringify({
      name: "Ticketmaster Malaysia",
      website_url: "https://www.ticketmaster.my/",
    }),
  });
...
async function upsertVenue(venue) {
  const name = venue?.name || "Malaysia";
...
function ticketmasterPrice(item) {
  const ranges = (item.priceRanges || [])
    .map((range) => ({
      ...
      currency: range.currency || "MYR",
    }))
    .filter((range) => range.min !== null || range.max !== null);

  if (!ranges.length) return { min: null, max: null, currency: "MYR" };

  const values = ranges.flatMap((range) => [range.min, range.max]).filter((value) => value !== null);
  return {
    min: Math.min(...values),
    max: Math.max(...values),
    currency: ranges.find((range) => range.currency)?.currency || "MYR",
  };
}
