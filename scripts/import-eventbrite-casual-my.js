...
async function fetchCasualEvents() {
  ...
  for (const page of CASUAL_PAGES) {
    const url = `https://www.eventbrite.com/d/malaysia--kuala-lumpur/${page.slug}/`;
...
async function upsertSource() {
  const rows = await supabaseFetch("/rest/v1/event_sources?on_conflict=name", {
    ...
    body: JSON.stringify({
      name: "Eventbrite Casual Malaysia",
      website_url: "https://www.eventbrite.com/d/malaysia--kuala-lumpur/free--events/",
    }),
  });
...
async function upsertVenue(location = {}) {
  ...
  const name = cleanText(location.name) || "Malaysia";
...
async function upsertEvent(item, page, startsAt, venue, source) {
  ...
  await supabaseFetch("/rest/v1/events?on_conflict=source_id,source_event_id", {
    ...
    body: JSON.stringify({
      ...
      description: summarize(item.description) || `Casual Malaysia activity from Eventbrite ${page.label}.`,
...
function venueAddress(location = {}) {
  ...
  return [
    ...
    address.addressCountry === "MY" ? "Malaysia" : address.addressCountry,
    address.postalCode,
  ].filter(Boolean).join(", ") || `${cleanText(location.name) || "Malaysia"}, Malaysia`;
}
